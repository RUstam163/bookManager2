...
//@EnableTransactionManagement — позволяет использовать TransactionManager для управления транзакциями. Hibernate
// работает с БД с помощью транзакций, они нужны чтобы какой-то набор операций выполнялся как единое целое, т.е. если в
// методе возникнут проблемы с какой-то одной операцией, тогда не выполнятся и все остальные, чтобы не было как в
// классическом примере с переводом денег, когда операция снятия денег с одного счета свершилась, а операция записи на
// другой не сработала, в итоге деньги исчезли.
...
//DataSource — используется для создания соединения с БД. Это альтернатива DriverManager, которой мы использовали ранее,
...
public class HibernateConfig {
    ...
    public LocalSessionFactoryBean sessionFactory() {
        ...
        sessionFactory.setPackagesToScan("bookManager.model");
